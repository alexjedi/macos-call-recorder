...
extension AppDelegate: NSMenuDelegate {
    func createMenu() {
        ...
        if streamType != nil { // recording?
            var typeText = ""
            if screen != nil {
                typeText = "Record Video from Display " + String((availableContent?.displays.firstIndex(where: { $0.displayID == screen?.displayID }))!+1)
            } else if window != nil {
                typeText = window?.owningApplication?.applicationName.uppercased() ?? "A window"
            } else {
                typeText = "Record Audio"
            }
            ...
            menu.addItem(header("Audio"))

            let audio = NSMenuItem(title: "Record Audio", action: #selector(prepRecord), keyEquivalent: "")
            audio.identifier = NSUserInterfaceItemIdentifier(rawValue: "audio")
            menu.addItem(audio)

            menu.addItem(header("Audio + Video"))
            ...
            for (i, display) in availableContent!.displays.enumerated() {
                ...
                displayItem.attributedTitle = NSAttributedString(string: "Record Video from Display \(i+1)" + (display.displayID == CGMainDisplayID() ? " (Main)" : ""))
...
//        menu.addItem(NSMenuItem(title: "Preferences…", action: #selector(openPreferences), keyEquivalent: ","))
        menu.addItem(NSMenuItem(title: "Quit Playbook AI", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q"))
...
//             }
    ...
    func updateIcon() {
        if let button = statusItem.button {
            button.image = NSImage(systemSymbolName: self.streamType != nil ? "record.circle.fill" : "record.circle", accessibilityDescription: "Playbook AI")
        }
    }
}
